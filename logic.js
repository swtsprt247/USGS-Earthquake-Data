...
d3.json(queryUrl, function (data) {
  ...
  d3.json(queryUrl2, function (data) {
    var faultLines = data.features;
    console.log(faultLines);
    createFeatures(earthquakeData, faultLines);
...
function createFeatures(earthquakeData, faultLineData) {
  ...
  // var FaultLines =  d3.json(queryUrl2, function (error, response) {
  //       if (error) return console.warn(error);

  //       console.log("url2", response);

  //       var faultLines = response.features.coordinates
...
function createMap(earthquakes, faultLines) {
  ...
  var myMap = L.map("map", {
    ...
    zoom: 5,
    layers: [streetMap, satelliteMap, outdoorsMap, earthquakes,faultLines]
  });
